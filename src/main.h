...
static const int COINBASE_MATURITY_PP = 500;
...
public:
    ...
    {
        // PPoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // PPoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // PPoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // PPoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // PPoin: whether transaction is a coinstake
    bool fCoinStake;

    // PPoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // PPoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // PPoin
            undo.nTime = nTime;            // PPoin
        }
        return true;
    }
...
public:
    ...
    // PPoin: block signature - signed by coin base txout[0]'s owner
    ...
    // PPoin: two types of block: proof-of-work or proof-of-stake
    ...
    // PPoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // PPoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // PPoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (PPoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // PPoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // PPoin: proof-of-stake related block index fields
    unsigned int nFlags;  // PPoin: block index flags
    enum
    {
