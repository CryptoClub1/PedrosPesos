...
class CoinContainer
  def create(options = {})
    ...
    bash_cmd += "./PedrosPesosd " + cmd_args.join(" ")

    if options[:remove_addr_after_shutdown]
      bash_cmd += "; rm -f /root/.PedrosPesos/testnet/peers.dat"
    end

    if options[:remove_wallet_after_shutdown]
      bash_cmd += "; rm /root/.PedrosPesos/testnet/wallet.dat"
    end
