...
// PPoin: remove transaction from mempool
...
// PPoin: reserve balance from being staked for network protection
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop PedrosPesos server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "PedrosPesos server stopping";
}
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use PedrosPesosd";
