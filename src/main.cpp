...
<<<<<<< HEAD
// Copyright (c) 2011-2015 The PedrosPesos developers
=======
...
int nCoinbaseMaturity = COINBASE_MATURITY_PP;
...
const string strMessageMagic = "PedrosPesos Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // PPoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // PPoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // PPoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // PPoin: subsidy is cut in half every 16x multiply of difficulty
...
// PPoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// PPoin: find last block index up to pindex
...
#endif
{
    ...
    // PPoin: target change every block
    // PPoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // PPoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // PPoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // PPoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // PPoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // PPoin
                    coins.nTime = undo.nTime;           // PPoin
                } else {
                    if (coins.IsPruned())
    ...
    // PPoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // PPoin: coinbase reward check relocated to CheckBlock()
    ...
    // PPoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // PPoin: fees are not collected by miners as in bitcoin
    // PPoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// PPoin: total coin age spent in transaction, in the unit of coin-days.
...
// PPoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // PPoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // PPoin: record proof-of-stake hash value
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PPoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // PPoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // PPoin: check that the block satisfies synchronized checkpoint
    ...
    // PPoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // PPoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // PPoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // PPoin: getblocks may not obtain the ancestor block rejected
    ...
    // PPoin: if responsible for sync-checkpoint send it
...
// PPoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // PPoin: calculate stake modifier checksum
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "PedrosPesos", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // PPoin: txindex is always enabled
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Harpaz 15-DEC-2017 Corporate Tax Pros Already Prepping for Tax Reform";
        CTransaction txNew;
        ...
        // PPoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // PPoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // PPoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // PPoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // PPoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // PPoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // PPoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // PPoin: record my external IP reported by peer
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // PPoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // PPoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // PPoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("PedrosPesosMiner : proof-of-work not meeting target");

    //// debug print
    printf("PedrosPesosMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PedrosPesosMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PedrosPesosMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    RenameThread(fProofOfStake? "PPoin-stake-minter" : "PPoin-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            // PPoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running PedrosPesosMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("PedrosPesosMiner terminated\n");
        throw;
    }
}
...
{
    // PPoin: mint proof-of-stake blocks in the background
